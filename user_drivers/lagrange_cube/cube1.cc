...
else if(L_prec_type == Exact_lsc_block_preconditioner)
{
  ...
  ////////////////////////////////////////////////////////////////////////////////
   ...
   // Get r_p
   this->get_block_vector(Doftype_list_vpl[N_velocity_dof_types],r,temp_vec);
   // passed
   ...
   this->return_block_vector(Doftype_list_vpl[N_velocity_dof_types],temp_vec,z);
   //pause("testo!");
///////////////////////////////////////////////////////////////////////////////
   ...
   for(unsigned i = 0; i < N_velocity_dof_types; i++)
   {
     ...
     //blockstringsteam << "vec_block_" << Doftype_list_vpl[i];
   ...
   for(unsigned i = 0; i< N_velocity_dof_types; i++)
   {
     ...
     this->return_block_vector(Doftype_list_vpl[i],another_temp_vec,z);
...
//   NS_preconditioner_pt->preconditioner_solve(x,y);
  ...
  //}
/*
  void set_nmeshes(const unsigned& nmesh)
  {
    cout << "setting nmesh: " << nmesh << endl;
    this->set_nmesh(nmesh);
    cout << "done nmesh" << endl;
  }
*/
...
/////////////////////////////////////////////////////////////////////////
                                           ...
                                           DoubleMatrixBase* matrix_pt)
 {
  ...
  // require only the (1) spatial dimension of the problem and (2) the number to 
  // dof types in each of the meshes. We assume the first mesh is always
  // the bulk mesh. Example:
  ...
  {
    this->set_mesh(mesh_i,problem_pt,problem_pt->mesh_pt(mesh_i));
  }
 ...
 // We get the spatial dimension straight from the sauce!
  FiniteElement* el_pt =  
    dynamic_cast<FiniteElement*> (problem_pt->mesh_pt(0)->element_pt(0));
  Dim = el_pt->dim();

  // determine the number of velocity dof types
  N_velocity_dof_types = nmesh*Dim;
  ...
  for(unsigned Mi=0; Mi<N_dof_types; Mi++)
  {
    for(unsigned Mj=0; Mj<N_dof_types; Mj++)
     {  
      ...
      blockname << "subjacblock_"<< currentsetting<< "_" 
                << setw(2) << setfill('0') << Mi 
                << setw(2) << setfill('0') << Mj;
...
//////////////////////////////////////////////////////////////////////////////
  ...
  for(unsigned dim_i = 0; dim_i < Dim; dim_i++)
  {
    ...
    for(unsigned mesh_i = 0; mesh_i < nmesh; mesh_i++)
    {
      ...
      // not fluid doftypes
      Doftype_list_vpl[incre_i] = mesh_i*Dim + dim_i + lagrange_offset;
      lagrange_offset += Ndoftype_in_mesh[mesh_i] - Dim;
      incre_i++;
    }
  }
  ...
  for(unsigned mesh_i = 0; mesh_i < nmesh; mesh_i++)
  {
    // This corresponds to getting the "one past" the w velocity, dim_i = Dim.
    // See the previous loop for details.
    unsigned doftype_begin = mesh_i*Dim + Dim + lagrange_offset;
    lagrange_offset = lagrange_offset + Ndoftype_in_mesh[mesh_i] - Dim;
    
    // Move to the end of the additional doftypes. Note that this corresponds
    // to the u velocity (dim_i = 0, see the previous loop).
    unsigned doftype_end = (mesh_i+1)*Dim + lagrange_offset;
...
// */
  ...
  // Get the momentum block corresponding to the u block.
  ...
  //       1) Create m * inv(w_i) * m
  //       2) Add the augmentation.
  ...
  for(unsigned l_i = 0; l_i < N_lagrange_dof_types; l_i++)
  {
    ...
    DenseMatrix<CRDoubleMatrix*> mm_pt(1,Dim,0);
    unsigned mm_i = 0;
    ...
    mm_locations.assign(Dim,0);
    // Required to increment through mm_locations.
    unsigned mm_locations_i = 0;
    ...
    for(unsigned col_i = 0; col_i < N_velocity_dof_types; col_i++)
    {
      ...
      if(mm_temp_pt->nnz() > 0)
      {
        ...
        mm_locations[mm_locations_i] = col_i;
        mm_locations_i++;

        // Store the mass marix.
        mm_pt(0,mm_i) = mm_temp_pt;
        mm_i++;
      }
      else
      {
   ...
   //pause("donecol"); 
    ...
    // in mm_pt. Now create the w_i and put it in w_pointers.
    
    // Get the number of local rows for this lagrange block.
    // We shall use the block in the first column.
    unsigned long l_i_nrow_local = mm_pt(0,0)->nrow_local();
    ...
    for(unsigned d_i = 0; d_i < Dim; d_i++)
    {
      if(mm_pt(0,d_i) != 0)
      {
        get_diag(mm_pt(0,d_i),m_diag[d_i]);
      }
    }
    ...
    unsigned long l_i_nrow_global = mm_pt(0,0)->nrow();
   ...
   for(unsigned ii = 0; ii < Dim; ii++)
   {
     ...
     for(unsigned jj = 0; jj < Dim; jj++)
     {
       unsigned aug_j = mm_locations[jj];
        ...
        if((mm_pt(0,ii)!=0) && (mm_pt(0,jj)!=0))
        {
        mm_pt(0,ii)->multiply((*inv_w_pt),(*aug_pt));
        aug_pt->multiply(*mm_pt(0,jj),(*aug_pt));
...
/* 
  ...
  // using the new Doftype_list_vpl, where i is 
  ...
  for(unsigned l_i = 0; l_i < N_lagrange_dof_types; l_i++)
  {
    ...
    for(unsigned row_i = 0; row_i < Dim; row_i++)
    {
      ...
      for(unsigned col_i = 0; col_i < Dim; col_i++)
      {
        ...
        // unnecessarily. This is because the multiply method requires
        // a matrix, not just a pointer.
        this->get_block(Doftype_list_vpl[constrained_row_i],
                        Doftype_list_vpl[constrained_col_i],
                        cr_matrix_pt,aug_pt);    
  ...
  for(unsigned row_i = 0; row_i < N_fluid_dof_types; row_i++)
  {
    for(unsigned col_i = 0; col_i < N_fluid_dof_types; col_i++)
    {
      ...
      blockname << "f_2aug_"<<row_i<<col_i;
      f_aug_ptrs(row_i, col_i)->sparse_indexed_output(blockname.str());
      delete f_aug_ptrs(row_i, col_i);
      f_aug_ptrs(row_i, col_i) = 0;
    }
  }
  ...
  f_aug_pt->sparse_indexed_output("f_aug2");
  delete f_aug_pt;
  f_aug_pt = 0;
  // */
...
////////////////////////////////////////////////////////////////////////////////
  ...
  // The velocity dof types are located at from 0 to 2*Dim-1 in Doftype_list_vpl.
  ...
  {
    unsigned required_block = Doftype_list_vpl[dof_type_i];
    assemble_inv_press_and_veloc_mass_matrix_diagonal
  ...
  for(unsigned l_i = 0; l_i < N_lagrange_dof_types; l_i++)
  {
    ...
    for(unsigned row_i = 0; row_i < Dim; row_i++)
    {
      ...
      for(unsigned col_i = 0; col_i < Dim; col_i++)
      {
        ...
        //cout << "Doftype_list_vpl[constrained_row_i] = " << Doftype_list_vpl[constrained_row_i]
        //     << "Doftype_list_vpl[constrained_col_i] = " << Doftype_list_vpl[constrained_col_i]
        //     << " " << endl;
        this->get_block(Doftype_list_vpl[constrained_row_i],
                        Doftype_list_vpl[constrained_col_i],
                        cr_matrix_pt,
...
///* 
 ...
 prec_pt->set_nmesh(this->nsub_mesh());
